...
final readonly class ProviderDefinition
{
    ...
    public static function fromArray(array $data): self
    {
        ...
        $adapter = strtolower(DefinitionData::string($data, 'adapter', $owner, true));
        $credentials = $data['credentials'] ?? [];
        if (!is_array($credentials)) throw new DefinitionException($owner . ' credentials must be a map of credential references.');
        ...
        return new self($name, $adapter, DefinitionData::string($data, 'model', $owner, true), $credentials, $options, is_array($data['metadata'] ?? null) ? $data['metadata'] : []);
    }
}
