...
class ScrollCodex extends EnvelopeCodex implements ScrollCodexContract
{
    ...
    public function registerScroll(
        ScrollContract $scroll,
    ): static {
        $this->validate($scroll);

        $this->scrolls[
            strtolower($scroll->name)
        ] = $scroll;
    ...
    public function ofType(
        ...
        $target = $type instanceof Types
            ? $type
            : Types::normalize($type, null);
        ...
        foreach ($this->scrolls as $name => $scroll) {
    ...
    public function withTag(
        ...
        foreach ($this->all(true) as $name => $scroll) {
    ...
    public function resolve(
        ...
        $key = strtolower(trim($uri));

        if ($key === '') {
            throw new InvalidArgumentException(
                'Scroll URI cannot be empty.',
            );
        }
        ...
        if ($this->has($key)) {
            ...
            if (!$scroll instanceof ScrollContract) {
                throw new LogicException(
                    sprintf(
                        'Resolved item [%s] is not a Scroll.',
                        $uri,
                    ),
                );
            }
    ...
    protected function hydrateFromEnvelope(
        \Codejitsu\Contracts\Envelope $envelope,
    ): mixed {
        if (!$envelope instanceof \Codejitsu\Contracts\Scrolls\Envelope) {
            throw new LogicException(
                'ScrollCodex can only hydrate Scroll envelopes.',
            );
        }

        $decoded = $this->decodeEnvelope($envelope);

        $scroll = $envelope->scrollType->make(
            $decoded,
        );
        ...
        if (!$scroll instanceof ScrollContract) {
            throw new LogicException(
                sprintf(
                    'Scroll type [%s] produced an invalid Scroll.',
